...
public class HandAnimator : MonoBehaviour
{
    ...
    [Header("Forced closed hand")]
    [SerializeField] private bool forceCloseHand;
    [SerializeField, Range(0, 1f)] private float forceCloseIntensity;
    ...
    private readonly List<Finger> closedHandFingers = new List<Finger>()
    {
        new Finger(FingerType.Thumb),
        new Finger(FingerType.Index),
        new Finger(FingerType.Middle),
    ...
    private void OnEnable()
    {
        ...
        {
            SetFingerAnimationValues(closedHandFingers, forceCloseIntensity);
            AnimateActionInput(closedHandFingers);
        }
    }
    ...
    {
        if(fingersToAnimate == null) return; 

        if(handAnimator == null) { this.handAnimator = GetComponent<Animator>(); }
